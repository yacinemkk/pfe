# =====================================================
# ENTRAÎNEMENT LSTM - Identification IoT
...
# =====================================================
# 3. COLONNES À SUPPRIMER (non SDN)
# =====================================================
COLS_TO_DROP = [
    "start",
    "srcMac",
...
# =====================================================
# 5. GÉNÉRATION D'ATTAQUES ADVERSARIALES IoT-SDN
# =====================================================


class IoTAdversarialAttack:
    """
    Attaque adversariale spécifique IoT-SDN selon la formulation:
    ...
    def _get_dependent_pairs(self):
        """Paires de features dépendantes"""
        pairs = []
    ...
    def _compute_class_centroids(self):
        """Calcule le centroid (μ) de chaque classe"""
        centroids = {}
    ...
    def _find_nearest_classes(self, k=3):
        """Trouve les k classes les plus proches pour chaque classe (L2 minimization)"""
        nearest = {}
        ...
        for class_idx in class_indices:
            ...
            # Distance euclidienne aux autres classes
    ...
    def _generate_masks(self, n_masks=20):
        """Génère les masques binaires les plus impactants (L0 minimization)"""
        ...
        # Masques avec différents pourcentages de features modifiées
        ...
        # Masques basés sur la variance des features
    ...
    def projection(self, X):
        """
        Fonction de projection pour maintenir les contraintes sémantiques
        - Arrondir les entiers
        - Préserver les plages valides
        - Maintenir la cohérence des features dépendantes
        """
        X_proj = X.copy()

        # Clip aux valeurs réalistes (après standardisation)
        X_proj = np.clip(X_proj, -3.0, 3.0)

        # Maintenir la cohérence des features dépendantes
        for indep_idx, dep_idx in self.dependent_pairs:
            # La feature dépendante devrait suivre une relation similaire
    ...
    def difference_function(self, mu_target, x0):
        """Calcule la différence entre le centroid cible et l'échantillon"""
        return np.abs(mu_target - x0)

    def generate_adversarial(self, model, X, y, max_iter=20, c=0.1):
        """
        Génère des exemples adversariaux selon la formulation IoT-SDN
        ...
        for i in range(n_samples):
            ...
            # Choisir une classe cible parmi les 3 plus proches
            if true_class in self.nearest_classes:
                ...
                # Choisir une classe aléatoire différente
            ...
            # Itération pour trouver la perturbation minimale
            x_adv = x0.copy()
            for t in range(1, max_iter + 1):
                ...
                # Vérifier si le modèle est trompé
            ...
            # Si pas de succès, garder la dernière perturbation
            X_adv[i] = x_adv
...
# =====================================================
# 8. PHASE 1: ENTRAÎNEMENT TRAFIC BÉNIN
# =====================================================
print("\n" + "=" * 60)
...
callbacks_phase1 = [
    ...
    ModelCheckpoint(
        "/content/drive/MyDrive/lstm_phase1_benin.h5",
        save_best_only=True,
        monitor="val_loss",
        verbose=1,
    ),
...
loss_p1, acc_p1 = model_lstm.evaluate(X_test_seq, y_test_cat, verbose=0)
print(f"\nPhase 1 - Loss: {loss_p1:.4f} | Accuracy: {acc_p1:.4f}")

# =====================================================
# 9. PHASE 2: ADVERSARIAL TRAINING IoT-SDN
# =====================================================
print("\n" + "=" * 60)
print("PHASE 2: ADVERSARIAL TRAINING IoT-SDN")
print("=" * 60)

# Initialiser le générateur d'attaques
...
X_adv_iot = attack_generator.generate_adversarial(
    model_lstm, X_train_seq[adv_indices], y_train_cat[adv_indices], max_iter=15, c=0.1
)
...
callbacks_phase2 = [
    ...
    ModelCheckpoint(
        "/content/drive/MyDrive/lstm_phase2_adversarial.h5",
        save_best_only=True,
        monitor="val_loss",
        verbose=1,
    ),
...
# Test données propres
loss_clean, acc_clean = model_lstm.evaluate(X_test_seq, y_test_cat, verbose=0)
print(f"\n[Propres] Loss: {loss_clean:.4f} | Accuracy: {acc_clean:.4f}")

# Test adversarial IoT-SDN
...
X_test_adv = attack_generator.generate_adversarial(
    model_lstm, X_test_seq[test_indices], y_test_cat[test_indices], max_iter=20, c=0.15
)

loss_adv, acc_adv = model_lstm.evaluate(X_test_adv, y_test_cat[test_indices], verbose=0)
print(f"[IoT-SDN Attack] Loss: {loss_adv:.4f} | Accuracy: {acc_adv:.4f}")

# Rapport classification
y_pred = model_lstm.predict(X_test_seq, verbose=0)
...
plt.title("Matrice de Confusion - LSTM (IoT IPFIX Home)")
plt.xlabel("Prédictions")
plt.ylabel("Vraies valeurs")
plt.xticks(rotation=45, ha="right")
plt.tight_layout()
plt.savefig("/content/drive/MyDrive/lstm_confusion_matrix.png", dpi=150)
plt.show()

# Courbes
fig, axes = plt.subplots(1, 2, figsize=(14, 5))

axes[0].plot(history_phase1.history["loss"], label="Train")
axes[0].plot(history_phase1.history["val_loss"], label="Validation")
axes[0].set_title("Phase 1 - Loss (Trafic Bénin)")
axes[0].set_xlabel("Epoch")
axes[0].set_ylabel("Loss")
axes[0].legend()

axes[1].plot(history_phase1.history["accuracy"], label="Train")
axes[1].plot(history_phase1.history["val_accuracy"], label="Validation")
...
plt.tight_layout()
plt.savefig("/content/drive/MyDrive/lstm_training_curves.png", dpi=150)
plt.show()
...
model_lstm.save("/content/drive/MyDrive/lstm_iot_sdn_final.h5")
print("Modèle: /content/drive/MyDrive/lstm_iot_sdn_final.h5")
...
with open("/content/drive/MyDrive/lstm_preprocessing.pkl", "wb") as f:
    pickle.dump(preprocessing, f)
print("Preprocessing: /content/drive/MyDrive/lstm_preprocessing.pkl")
