...
# ─── Nouvelles Contre-Mesures Adversariales (4 Couches) ────────────────
...
USE_MULTI_ATTACK = True    # Worst-of-K Multi-Attack TRADES (PGD + Zero/Mimic/Padding)
MULTI_ATTACK_STRATEGIES = ['Zero', 'Mimic_Mean', 'Mimic_95th', 'Padding_x10']

# ─── Couche 4: IBP (Interval Bound Propagation) — Certified Robustness ──────
USE_IBP           = True    # Couche 4: Interval Bound Propagation (certified robustness)
IBP_EPSILON       = 0.3     # IBP ε-scaling factor (0=none, 1=full data-driven box)
LAMBDA_IBP        = 1.0     # IBP loss weight
IBP_METHOD        = 'crown' # IBP bound method: 'crown' (tight) or 'ibp' (fast)
IBP_EPSILON_START = 0.05    # IBP ε at start of warmup (ramps to IBP_EPSILON)
IBP_WARMUP_EPOCHS = 5       # IBP warmup epochs for λ and ε scaling
...
def train_model_with_countermeasures(
    ...
    Couche 2: TRADES (Worst-of-K Multi-Attack: PGD + Zero/Mimic/Padding) + Label Smoothing
    Couche 3: Randomized Smoothing (eval only)
    Couche 4: IBP (Interval Bound Propagation) — Certified Robustness (Phase 2 only)
    Phase 1: Standard training with AFD + InputTransform + CutMix
    Phase 2: Adversarial training with TRADES + IBP + all countermeasures
    """
    ...
    print(f"  Couche 3 - RS: {use_randomized_smoothing} (σ={rs_sigma})")
    print(f"  Couche 4 - IBP: {use_ibp} (ε={ibp_epsilon}, λ={lambda_ibp}, method={ibp_method})")
    print(f"{'='*80}\n")
    ...
    # ── INTER-PHASE CRASH TEST ──────────────────────────────────────────────────
    print(f"\n{'─'*80}")
    print(f"  CRASH TEST — After Phase 1 (pré-durcissement adversarial)")
    print(f"{'─'*80}\n")

    model.eval()

    # 1. Clean accuracy on validation set
    _ct_correct, _ct_total = 0, 0
    with torch.no_grad():
        for _ct_X, _ct_y in val_loader:
            _ct_X, _ct_y = _ct_X.to(device), _ct_y.to(device)
            _ct_out = model(_ct_X)
            _, _ct_pred = _ct_out.max(1)
            _ct_total += _ct_y.size(0)
            _ct_correct += _ct_pred.eq(_ct_y).sum().item()
    _ct_clean_acc = _ct_correct / _ct_total
    print(f"  [P1 Crash Test] Clean Accuracy          : {_ct_clean_acc:.4f}")

    # 2. PGD adversarial accuracy (small subset for speed)
    _ct_pgd_attack = TRADESAttack(epsilon=epsilon, alpha=alpha, num_steps=pgd_steps)
    _ct_pgd_correct, _ct_pgd_total = 0, 0
    _ct_n_batches = 0
    for _ct_X, _ct_y in val_loader:
        if _ct_n_batches >= 10:          # evaluate on ~10 batches for speed
            break
        _ct_X, _ct_y = _ct_X.to(device), _ct_y.to(device)
        with torch.no_grad():
            _ct_X_adv = _ct_pgd_attack.generate(model, _ct_X, _ct_y, device)
        with torch.no_grad():
            _ct_out = model(_ct_X_adv)
        _, _ct_pred = _ct_out.max(1)
        _ct_pgd_total += _ct_y.size(0)
        _ct_pgd_correct += _ct_pred.eq(_ct_y).sum().item()
        _ct_n_batches += 1
    _ct_pgd_acc = _ct_pgd_correct / max(_ct_pgd_total, 1)
    _ct_pgd_rr  = _ct_pgd_acc / max(_ct_clean_acc, 1e-8)
    print(f"  [P1 Crash Test] PGD Adversarial Accuracy: {_ct_pgd_acc:.4f}  (RR={_ct_pgd_rr:.4f})")

    # 3. Worst-of-K adversarial accuracy (if multi-attack ready)
    if use_multi_attack and sensitivity_analysis is not None:
        _ct_wok_correct, _ct_wok_total = 0, 0
        _ct_n_batches = 0
        for _ct_X, _ct_y in val_loader:
            if _ct_n_batches >= 10:
                break
            ...
            with torch.no_grad():
                _ct_out = model(_ct_X_adv)
            _, _ct_pred = _ct_out.max(1)
            _ct_wok_total += _ct_y.size(0)
            _ct_wok_correct += _ct_pred.eq(_ct_y).sum().item()
            _ct_n_batches += 1
        _ct_wok_acc = _ct_wok_correct / max(_ct_wok_total, 1)
        _ct_wok_rr  = _ct_wok_acc / max(_ct_clean_acc, 1e-8)
        print(f"  [P1 Crash Test] Worst-of-K Accuracy     : {_ct_wok_acc:.4f}  (RR={_ct_wok_rr:.4f})")

    print(f"\n  → Phase 2 will apply adversarial hardening on these weaknesses.\n")
    del _ct_pgd_attack
    if torch.cuda.is_available():
        torch.cuda.empty_cache()
    # ────────────────────────────────────────────────────────────────────────────

    # ── PHASE 2: Adversarial Training with 4 Countermeasures ──
    print(f"\n{'─'*80}")
    ...
    if use_multi_attack and sensitivity_analysis is not None:
        ...
        print(f"  ⚠️ Multi-attack requested but no sensitivity_analysis. Falling back to PGD only.")
    ...
    if use_ibp:
        ...
        if sensitivity_analysis is not None:
            ...
            try:
                _ibp_non_mod = [sensitivity_analysis.feature_names.index(n) for n in sensitivity_analysis.non_modifiable if n in sensitivity_analysis.feature_names]
            except (ValueError, AttributeError):
                _ibp_non_mod = []
        ...
        if _ibp_benign_stats is not None:
            print(f"  ✅ IBP trainer created (data-driven+ε-scaling: ε {ibp_epsilon_start}→{ibp_epsilon}, method={ibp_method})")
        else:
            print(f"  ✅ IBP trainer created (ε={ibp_epsilon}, method={ibp_method})")
    ...
    best_state = None
    patience_counter = 0
    is_multi_attack = isinstance(active_attack, MultiAttackTRADES)
    ...
    for epoch in range(epochs_phase2):
        ...
        epoch_msg = f"  P2 Epoch {epoch+1}/{epochs_phase2} | Acc: {train_acc:.4f} | Loss: {train_loss/len(train_loader):.4f} | CE: {avg_ce:.4f} | KL: {avg_kl:.4f} | Val: {val_loss:.4f}"

        # Print worst-of-K selection stats
        if is_multi_attack:
            stats = active_attack.get_selection_stats()
            freq_str = " | ".join(f"{name}: {s['frequency']:.1%}" for name, s in stats.items())
            epoch_msg += f"\n  [Worst-of-K] {freq_str}"
